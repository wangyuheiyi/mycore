...
public abstract class ServerConfig implements Config
{
	...
	public void validate()
	{
		if(serverType < 1) 
		{
			throw new IllegalArgumentException("The serverType must not be empty");
		}

		if(StringUtils.isEmpty(this.regionId)|| StringUtils.isEmpty(this.serverGroupId))
		{
			throw new IllegalArgumentException("The regionId,serverGroupId and the serverIndexId must not be empty");
		}
		if(StringUtils.isEmpty(this.localHostId)|| StringUtils.isEmpty(this.serverId))
		{
			throw new IllegalArgumentException("The localHostId and the serverId must not be empty");
		}
		...
		if (serverType == 1)
		{
			...
			if (this.logConfig.logServerIp == null
				|| (this.logConfig.logServerIp = logConfig.logServerIp.trim()).length() == 0)
			{
				throw new IllegalArgumentException("The logsConfig.logServerIp must not be null.");
			}

			if (this.logConfig.logServerPort <= 0) 
			{
				throw new IllegalArgumentException("The logsConfig.logServerPort must be greater than 0.");
			}
		}
	}
	...
	public int getApnOpen() {
		return apnOpen;
	}

	public void setApnOpen(int apnOpen) {
		this.apnOpen = apnOpen;
	}
